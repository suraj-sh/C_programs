...
// WAP to print all characters in english alphabet using a pointer
#include <stdio.h>

void main() {
    char ch = 'a';      
    char *ptr = &ch;        // stores address of variable ch in ptr which is currently a 

    while(*ptr <= 'z'){
        printf("%c ", *ptr);
        (*ptr)++;           // increments a until z 
    }
}
